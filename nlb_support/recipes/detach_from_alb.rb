...
ruby_block "detach from NLB" do
  block do
    ...
    connection_draining_timeout = node[:nlb_helper][:connection_draining_timeout]
    state_check_frequency = node[:nlb_helper][:state_check_frequency]
    ...
    Chef::Log.info("Creating NLB client in region #{stack_region}")
    ...
    if connection_draining_timeout == 0
      ...
      Chef::Log.info("delaying execution recipes until instance is drained from NLB or timeout of #{connection_draining_timeout} seconds elapses")
      start_time = Time.now
      loop do
        ...
        Chef::Log.info("state of instance in NLB: #{target_health_state}")
